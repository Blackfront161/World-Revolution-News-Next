...
Status: Profile angelegt; bisherige G1-/G2-/G3-001-bis-G3-006-Instanzen
abgeschlossen; WRN-G3-007 ist nur dokumentarisch vorbereitet und aktuell ist
kein Mitarbeiter aktiv. Laufstatus steht verbindlich in
...
### Fachleads ab WRN-GOV-001

Die Profile `frontend_brand_engineer` und
`backend_data_reliability_engineer` koennen bei ausdruecklicher Freigabe im
Elternbrief als Fachleads arbeiten. Es entstehen keine neuen Managerprofile.
Ohne `Weiterdelegation: erlaubt` bleiben sie direkt ausfuehrende Spezialisten.
Main -> Fachlead -> Helfer ist die maximale Tiefe; Helfer delegieren nicht.
QA, Security und Architekturreview berichten unabhaengig direkt an Main/Chief.
Der operative Vertrag steht in `docs/10-AGENT-ORCHESTRATION.md` und ersetzt
keine engeren Produktslicegates.
...
5. Innerhalb der aktuellen Runtime sind global bis zu drei Subagenten inklusive
   Nachkommen und wartender offener Instanzen erlaubt. Jeder Slot braucht einen
   unabhaengigen Scope; maximal zwei duerfen gleichzeitig Produktcode in
   disjunkten Dateien und Vertraegen schreiben. Der dritte Slot ist fuer
   read-only Analyse, QA, Security, Dokumentation oder disjunkte Testevidenz.
   Die niedrigere Taskgrenze gilt. Main/Chief reserviert zentral.
