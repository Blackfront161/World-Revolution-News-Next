...
Innerhalb der aktuellen Runtime darf der Chief bis zu drei Subagenten neben
dem Main zentral reservieren. Maximal zwei schreiben gleichzeitig an
disjunktem Produktcode; Slot drei dient read-only Analyse, QA, Security,
Dokumentation oder disjunkter Testevidenz. Zentrale Reservierung und begrenzte
Weiterdelegation folgen `docs/10-AGENT-ORCHESTRATION.md` (WRN-GOV-001).
Parallele Schreibarbeit am selben Vertrag ist verboten.
Ein groesseres Modell wird nur bei hoeherem Risiko oder
nachgewiesener Unklarheit eingesetzt, nicht wegen fehlender Vorbereitung.

Fachleads erhalten keinen eigenen Kosten-/Slotpool. Ein spaeterer Pilot bindet
vorab Arbeitsumfang, Checkpoint und Versuchsgrenze. Nacharbeit, Koordination,
Defekte und werkzeugbelegte Tokenwerte werden berichtet; fehlende Werte
bleiben unbekannt. Promptgrenzen sind keine technisch garantierten Hard Caps.
Die Organisationsentscheidung nimmt keine offenen Providerbudgets vorweg.
...
## 3.1 Bevorzugte Mehrmodell-Besetzung

| Situation | Slot A | Slot B | Slot C |
|---|---|---|---|
| Vorbereitung | Luna Inventar/Kontinuitaet | Sol Architektur/Vertrag | Terra technische Machbarkeit read-only |
| Getrennte Implementierung | Terra Frontend | Terra Backend/Data | Luna Register/Evidence read-only |
| Eingefrorenes Gate | Terra QA/A11y | Sol Security/Architektur | Luna Evidence-/Handoffabgleich |
| Schwieriger Incident | Sol Ursachenanalyse | Terra Reproduktions-/Integrationsowner | Luna Log-/Chronologieabgleich |

Diese Besetzungen sind Obergrenzen, keine Pflicht zur Slotfuellung. Ein Agent
wird nur gestartet, wenn sein Teilauftrag unabhaengig und nuetzlich ist.
Security und Architektur duerfen denselben eingefrorenen Kandidaten parallel
lesen; bei spaeterer Produktmutation ist ein enger Recheck erforderlich.
