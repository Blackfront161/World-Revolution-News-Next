...
Status: `ACCEPTED` – G2-Abnahme am 23. August 2026; WRN-G3-001 bis WRN-G3-010
lokal technisch, unabhaengig und visuell akzeptiert; WRN-G3-011-Kandidat
`d19ce4d` implementiert den lokalen Lesestatus samt Rollbackschutz und die
unabhaengige Re-QA `f1ebf70` ist GREEN; den aktuellen Task-/Abnahmestand
fuehren ausschliesslich Source-of-Truth und Project State
...
| Brand/Design System | Tokens, freigegebene Assets, geeignete atomare Komponenten, Accessibility-Grundregeln, versionierte Theme-IDs und semantische Markenakzente | keine gemeinsame Navigation oder identische Clientkomposition erzwingen |
| UI Language/Copy | typisierte schluesselgleiche lokale Kataloge fuer en/de/es/fr/it/pt/ru/el/tr; kanonischer englischer Vertrag | keine Inhaltsuebersetzung, kein HTML, kein dynamischer Pfad aus Storage, keine Providerkopplung |
...
9. **Themes:** sechs semantische Farbpaletten koennen beide Clients teilen;
   `system` bleibt eine validierte Nutzerpraeferenz, die nur Hell/Dunkel
   aufloest. Persistenz und Systembeobachtung bleiben clientlokal. Headerlayout
   und Platzierung der Auswahl duerfen plattformspezifisch sein.
...
## 8. Organisationsvertrag (WRN-GOV-001)

Fachleads spiegeln die Frontend-/Backendgrenzen, ohne die Produktarchitektur
oder ihre Freigaben zu veraendern. Bestehende Profile, zentrale Slotvergabe,
begrenzte Weiterdelegation und unabhaengige QA folgen
`docs/10-AGENT-ORCHESTRATION.md`. Engere aktuelle Produkttaskgates haben
Vorrang; die Organisationsregel startet insbesondere keinen Produktslice.
