...
describe('website UI-neutral controller result and lifecycle contract', () => {
  it('every public async action returns the same complete immutable nullable projection', async () => {
    ...
    const controller = createWebsiteContentOfflineController();
    for (const action of [
      'restore',
      'save',
      'check',
  ...
  it('a pre-aborted operation does not open a database or invoke source loading', async () => {
    ...
    const check = vi.fn();
    const controller = createWebsiteContentOfflineController({ openStore, check });
  ...
  it('dispose consumes a late rejected open and does not retry', async () => {
    ...
    const controller = createWebsiteContentOfflineController({ openStore });
    const pending = controller.restore();
    controller.dispose();
