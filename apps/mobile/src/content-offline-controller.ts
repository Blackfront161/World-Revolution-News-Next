...
import {
  ...
  type OfflineOperation as Operation,
...
export function createMobileContentOfflineController(
  overrides: Partial<ControllerDependencies> = {},
) {
  let sequence = 0;
  const dependencies: ControllerDependencies = {
    openStore: openMobileContentOfflineStore,
    check: checkLocalContentReleaseForOffline,
    now: () => Date.now(),
    operationId: () => `mobile-content-offline-${++sequence}`,
    ...overrides,
  };
  let connection: ContentOfflineStore | null = null;
  const operations = createOfflineOperationScope(() => new ContentOfflineStoreError('aborted'));
  ...
  const run = async (
    ...
    const op = operations.begin(callerSignal);
    ...
    try {
      ...
      if (op.controller.signal.aborted || operations.disposed)
        return result(
          operations.disposed ? 'disposed' : 'aborted',
          op.reason,
          { failure: op.reason },
          op,
        );
  ...
  const write = async (
    ...
    return op.wait(
      action().then((control) => {
        ...
        if (operations.isCurrent(op)) remember(control);
        return control;
      }),
    );
  };
