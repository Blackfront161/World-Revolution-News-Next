...
import {
  createMobileProductionTranslationAdapter,
...
describe('mobile production translation adapter', () => {
  it('defaults unavailable and rejects incomplete or unsafe configuration', () => {
    ...
    for (const candidate of [
      ...
      expect(createMobileProductionTranslationAdapter(candidate)).toBeNull();
  });
  it('sends only exact public paragraph fields and checks the cache-labelled response', async () => {
    ...
    const adapter = createMobileProductionTranslationAdapter(config, {
      fetch,
      now: () => now,
      online: () => true,
    })!;
  ...
  it('performs no request for same language, offline, cancelled or oversize input', async () => {
    ...
    const offline = createMobileProductionTranslationAdapter(config, {
      fetch,
      now: () => now,
      online: () => false,
    })!;
  ...
  it('settles on caller abort and discards a late uncooperative fetch without another request', async () => {
    ...
    const adapter = createMobileProductionTranslationAdapter(config, { fetch, now: () => now })!;
